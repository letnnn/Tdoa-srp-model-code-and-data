...
def estimate_time_delay(audio1, audio2, fs):
    ...
    # 找到互相关函数的峰值位置并进行三点抛物线拟合
    maxIndex = np.argmax(np.abs(R12_phat))
    refinedMaxIndex = refine_peak(maxIndex, R12_phat)
...
def refine_peak(maxIndex, R12_phat):
    """使用三点抛物线拟合来细化峰值位置"""
    if 0 < maxIndex < len(R12_phat) - 1:
        y = np.abs(R12_phat[maxIndex-1:maxIndex+2])
        x = np.array([-1, 0, 1])
        p = np.polyfit(x, y, 2)
        peakOffset = -p[1] / (2 * p[0])
        return maxIndex + peakOffset
    return maxIndex
...
def estimate_time_delay_matrix(audio_files, fs):
    ...
    for i in range(num_mics):
        for j in range(i + 1, num_mics):
            timeDelay = estimate_time_delay(audio_files[i], audio_files[j], fs)
            timeDelayMatrix[i, j] = round(timeDelay, 8)
            timeDelayMatrix[j, i] = -timeDelayMatrix[i, j]
...
def residuals_func(estimated_position, mic_positions, time_delay_matrix, c):
    ...
    for i in range(1, mic_positions.shape[0]):
        d0i_estimated = np.linalg.norm(estimated_position - mic_positions[0]) - np.linalg.norm(estimated_position - mic_positions[i])
        d0i_actual = time_delay_matrix[0, i] * c
        residuals.append(d0i_estimated - d0i_actual)
    return np.array(residuals)
...
def estimate_source_position(mic_positions, time_delay_matrix, c):
    """使用最小二乘法求解非线性方程，估计声源位置"""
    initial_guess = np.mean(mic_positions, axis=0)
    bounds = (np.array([-2, -2, 0]), np.array([2, 2, 2]))
    result = least_squares(residuals_func, initial_guess, args=(mic_positions, time_delay_matrix, c), bounds=bounds)
    return result.x

def load_audio_files(file_paths):
    """加载多个音频文件，返回音频数据和采样频率"""
    audio_files = []
    fs = None
...
# 测试代码
audio_files, fs = load_audio_files(['1/1/音频 1-3.wav', '1/1/音频 1-4.wav', '1/1/音频 1-5.wav', '1/1/音频 1-6.wav'])
...
# 计算声源到原点（0,0,0）的距离
distance_to_origin = np.linalg.norm(estimated_position)

# 打印声源到原点的距离
print(f"声源到原点的距离: {distance_to_origin:.10f} 米")
